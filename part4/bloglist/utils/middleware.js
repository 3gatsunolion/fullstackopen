...
// error-handling middleware function. Even if you don’t need to use the next object, you must specify it to
...
const userExtractor = async (request, response, next) => {
  if (request.token) {
    ...
    if (decodedToken.id){
      ...
      if (!request.user) {
        request.authError = { status: 400, error: 'UserId missing or not valid' }
      }
    } else {
      request.authError = { status: 401, error: 'token invalid' }
    }
  }
