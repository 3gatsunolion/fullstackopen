...
blogsRouter.post("/", async (request, response) => {
  ...
  if (!request.user) {
    return response
      .status(request.authError.status)
      .json({ error: request.authError.error });
  }
...
blogsRouter.delete("/:id", async (request, response) => {
  ...
  if (blogToDelete.user.toString() === user.id.toString()) {
    ...
    return response
      .status(403)
      .json({ error: "Forbidden: You cannot delete this blog" });
  }
});
...
blogsRouter.put("/:id", async (request, response) => {
  ...
  const updatedBlog = await blogToModify
    .save()
    .then((b) => b.populate("user", { username: 1, name: 1 }));
