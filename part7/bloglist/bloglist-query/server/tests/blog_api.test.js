...
describe("when there is initially some blogs saved", () => {
  ...
  describe("deletion of a blog", () => {
    let token;
    let user = helper.sampleUsers[1];

    beforeEach(async () => {
      token = await loginAndGetToken(user);
    });
  ...
  describe("modification of a blog", () => {
    ...
    test("all fields will be updated if id is valid", async () => {
      ...
      const savedModifiedBlog = blogsAtEnd.find(
        (blog) => blog.id === blogToModify.id,
      );
  ...
  test("a comment can be added to a blog", async () => {
    ...
    assert.strictEqual(updatedBlog.comments.length, 1);
    assert.strictEqual(updatedBlog.comments[0], newComment.comment);
  });
});
